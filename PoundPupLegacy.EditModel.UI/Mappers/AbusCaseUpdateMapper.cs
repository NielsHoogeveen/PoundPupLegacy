    ...
    ) : IMapper<ExistingAbuseCase, ImmediatelyIdentifiableAbuseCase>
{
    public ImmediatelyIdentifiableAbuseCase Map(ExistingAbuseCase viewModel)
    {
        ...
        return new CreateModel.ExistingAbuseCase {
            ...
            TenantNodesToAdd = newTenantNodeMapper.Map(viewModel.TenantNodesToAdd).ToList(),
            TenantNodesToRemove = tenantNodeToRemoveMapper.Map(viewModel.TenantNodesToUpdate).ToList(),
            TenantNodesToUpdate = tenantNodeToUpdateMapper.Map(viewModel.TenantNodesToUpdate).ToList(),
