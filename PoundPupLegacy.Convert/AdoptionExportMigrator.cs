...
internal sealed class AdoptionExportMigrator: Migrator
{
    public AdoptionExportMigrator(MySqlToPostgresConverter mySqlToPostgresConverter) : base(mySqlToPostgresConverter)
    {
    }

    protected override string Name => "adoption exports";

    private int GetAdoptionExportRelationId(int countryIdTo, int? countryIdFrom, string? countryNameFrom)
    {
        const string sql1 = """
    ...
    {
            await AdoptionExportRelationCreator.CreateAsync(ReadAdoptionExportRelations(), _postgresConnection);
            await AdoptionExportYearCreator.CreateAsync(ReadAdoptionExportYears(), _postgresConnection);

    }
    private async IAsyncEnumerable<AdoptionExportRelation> ReadAdoptionExportRelations()
    {
        ...
        while (await reader.ReadAsync())
        {
            ...
            {
                CountryIdTo = await _nodeIdReader.ReadAsync(Constants.PPL, reader.GetInt32("country_id_to")),
                CountryIdFrom = reader.IsDBNull("country_id_from") ? null : await _nodeIdReader.ReadAsync(Constants.PPL, reader.GetInt32("country_id_from")),
    ...
    private async IAsyncEnumerable<AdoptionExportYear> ReadAdoptionExportYears()
    {
        ...
        while (await reader.ReadAsync())
        {
            var country = new AdoptionExportYear
            {
                AdoptionExportRelationId = GetAdoptionExportRelationId(
                    await _nodeIdReader.ReadAsync(Constants.PPL, reader.GetInt32("country_id_to")),
                    reader.IsDBNull("country_id_from") ? null : await _nodeIdReader.ReadAsync(Constants.PPL, reader.GetInt32("country_id_from")),
