...
internal sealed class UnitedStatesCongressionalMeetingDocumentReaderFactory : SingleItemDatabaseReaderFactory<Request, UnitedStatesCongressionalMeeting>
{
        ...
        basic_nameable_document AS (
            ...
            FROM (
                SELECT
                    ...
                    lower(bn.date_range) date_from,
                    upper(bn.date_range) date_to,
                    bn.number
                FROM node n
                join tenant_node tn on tn.node_id = n.id
                join nameable nm on nm.id = n.id 
                join united_states_congressional_meeting bn on bn.id = n.id
    ...
    protected override UnitedStatesCongressionalMeeting? Read(NpgsqlDataReader reader)
    {
        return reader.GetFieldValue<UnitedStatesCongressionalMeeting>(0);
    }
}
