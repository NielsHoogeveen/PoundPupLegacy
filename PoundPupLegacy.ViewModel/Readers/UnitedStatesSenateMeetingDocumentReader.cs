...
using Request = UnitedStatesSenateMeetingDocumentReaderRequest;

public sealed record UnitedStatesSenateMeetingDocumentReaderRequest : IRequest
{
    public required int Number { get; init; }
    public required int TenantId { get; init; }
}
internal sealed class UnitedStatesSenateMeetingDocumentReaderFactory : SingleItemDatabaseReaderFactory<Request, CongressionalMeetingChamber>
{
    ...
    const string SQL = $"""
        ...
        party_affiliations as(
        	select
        	tpa.congressional_term_id senate_term_id,
        	jsonb_agg(
        		jsonb_build_object(
        			'Title',
        			n.title,
        ...
        bills as(
        	...
        	join node_type nt on nt.id = n.node_type_id
        	join senator_senate_bill_action sba on sba.senate_bill_id = sb.id
        ...
        from(
        	select
        	'United States Senate - ' || n3.title meeting_name,
        	...
        	jsonb_agg(
        		jsonb_build_object(
        			...
        			'/attachment/' || f.id,
        			'Bills',
        			(select "document" from bills b where b.senator_id = s.id),
        			'Letters',
        			(select "document" from letters l where l.person_id = p.id),
        			'Parties',
        			(select "document" from party_affiliations pa where pa.senate_term_id = st.id)
        		)
        	) senators
        	from person p
