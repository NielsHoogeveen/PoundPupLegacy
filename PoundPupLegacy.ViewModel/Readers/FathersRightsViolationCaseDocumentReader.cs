...
internal sealed class FathersRightsViolationCaseDocumentReaderFactory : SingleItemDatabaseReaderFactory<Request, FathersRightsViolationCase>
{
    ...
    const string SQL = $"""
            ...
            {BREADCRUM_DOCUMENT},
            {DOCUMENT}
            SELECT document from fathers_rights_violation_case_document
            """
            ;
    ...
    protected override FathersRightsViolationCase? Read(NpgsqlDataReader reader)
    {
        return reader.GetFieldValue<FathersRightsViolationCase>(0);
    }
}
