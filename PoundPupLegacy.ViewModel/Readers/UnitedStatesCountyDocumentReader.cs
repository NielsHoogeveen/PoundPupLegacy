...
internal sealed class UnitedStatesCountyDocumentReaderFactory : SingleItemDatabaseReaderFactory<Request, UnitedStatesCounty>
{
    ...
    const string SQL = $"""
            ...
            {BREADCRUM_DOCUMENT},
            {DOCUMENT}
            SELECT document from united_states_county_document
            """
            ;

    const string DOCUMENT = """
        united_states_county_document AS (
            SELECT 
                jsonb_build_object(
                    'NodeId', n.node_id,
                    ...
                    jsonb_build_object(
                        'Path', 
                        '/' || united_states_state_viewer_path || '/' || n.united_states_state_id,
                        'Title', 
                        n.united_states_state_title
                    ),
                    'Fips',
                    fips,
            ...
            FROM (
                SELECT
                    ...
                    n2.id united_states_state_id,
                    n2.title united_states_state_title,
                    nt2.viewer_path united_states_state_viewer_path,
                    bn.fips
                FROM node n
                join tenant_node tn on tn.node_id = n.id
                join nameable nm on nm.id = n.id 
                join united_states_county bn on bn.id = n.id
                join node n2 on n2.id = bn.united_states_state_id
    ...
    protected override UnitedStatesCounty? Read(NpgsqlDataReader reader)
    {
        return reader.GetFieldValue<UnitedStatesCounty>(0);
    }
}
