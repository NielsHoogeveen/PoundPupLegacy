...
internal sealed class SenateBillDocumentReaderFactory : SingleItemDatabaseReaderFactory<Request, SenateBill>
{
    ...
    const string DOCUMENT = """
        basic_nameable_document AS (
            SELECT 
                jsonb_build_object(
                    ...
                    (
        				...
        				jsonb_agg(
                            jsonb_build_object(
                                'Senator', jsonb_build_object(
                                    'Path', 
                                    '/' || senator_viewer_path || '/' || senator_id,
                                    'Title', 
                                    senator_name
                                ),
                        ...
                        FROM (
                            SELECT 
        					hba.senate_bill_id node_id,
                            p.id senator_id,
                            n.title senator_name,
                            bat.id bill_action_type_id,
                            n2.title bill_action_type_name,
                            hba.date,
                            nt.viewer_path senator_viewer_path,
                            nt2.viewer_path bill_action_type_viewer_path
                            FROM senator_senate_bill_action hba
                            JOIN senator s on s.id = hba.senator_id
                            ...
                            where hba.senate_bill_id = @node_id
                        ) bill_actions
                    ),
            ...
            FROM (
                ...
                join senate_bill sb on sb.id = n.id
    ...
    protected override SenateBill? Read(NpgsqlDataReader reader)
    {
        return reader.GetFieldValue<SenateBill>(0);
    }
}
