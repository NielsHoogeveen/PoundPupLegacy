...
using Request = ChildTraffickingCasesDocumentReaderRequest;

public sealed record ChildTraffickingCasesDocumentReaderRequest : IRequest
{
...
internal sealed class ChildTraffickingCasesDocumentReaderFactory : SingleItemDatabaseReaderFactory<Request, ChildTraffickingCases>
{
    ...
    private static readonly FieldValueReader<ChildTraffickingCases> DocumentReader = new() { Name = "document" };
        		    ...
        		    from(
                        ...
                        from(
        			        ...
        			        from(
                                ...
                                ntm2.tag_label_name term_type_name,
        				        case 
                                ...
                                left join nameable_type ntm2 on ntm2.id = n2.node_type_id and ntm2.tag_label_name is not null
            ...
            from(
        	    ...
        	    from(
                    ...
                    from(
                        ...
                        join child_trafficking_case ac on ac.id = n2.id
        			    ...
        			    and (@terms is null or n2.id in (
                            ...
                            from (
                                ...
                                count(*) over() c
                                from term t
                                left join node_term nt on nt.term_id = t.id and ntm.node_id = n2.id
                                where t.id = ANY(@terms)
                            ) x
                            group by node_id, c
                            having count(node_id) = c
    ...
    protected override ChildTraffickingCases Read(NpgsqlDataReader reader)
    {
