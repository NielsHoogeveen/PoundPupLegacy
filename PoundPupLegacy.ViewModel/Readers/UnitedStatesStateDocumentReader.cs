...
internal sealed class UnitedStatesStateDocumentReaderFactory : SingleItemDatabaseReaderFactory<Request, UnitedStatesState>
{
    ...
    const string SQL = $"""
            ...
            {DOCUMENT}
            SELECT document from united_states_state_document
            """
            ;

    const string DOCUMENT = """
        united_states_state_document AS (
            SELECT 
                jsonb_build_object(
                'NodeId', n.node_id,
            ...
            FROM (
                ...
                join united_states_state uss on uss.id = sd.id
    ...
    protected override UnitedStatesState? Read(NpgsqlDataReader reader)
    {
        return reader.GetFieldValue<UnitedStatesState>(0);
    }
}
